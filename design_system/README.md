...
| Page header | `page-header.html` | Mobile/tablet screen title (h2 24/600 mobile · 28/600 tablet) · optional leading + trailing CircleButton(36) · no fill, no border. Desktop uses the content-pane h1 header. |
| Content pane (web) | `content-pane.html` | Page-on-scaffold standard: page header (h1 32/700 + subtitle) on scaffold · optional KPI strip · table-only surface card. Replaces wrapped-in-card title. |
| Destination card | `destination-card.html` | Workspace top-row entry · gradient surface→surface-container-low · radius med · icon 28 + label bodyLarge · ~150×104 · Coming Soon / Beta badges |
| Block card | `block-card.html` | Term/block card · surface · radius med · status pill (accent2 bg + accent3) · title bodyLarge · StatChip row |
| Stat chip | `stat-chip.html` | Filled tertiary info pill · radius small · 16px icon + label · on-surface-variant |
| Switch / Toggle | `switch.html` | Track 44×26 · knob 22 · on primary / off outline-variant (no canonical Flutter Switch — proposed) |
| Row action menu | `row-action-menu.html` | Per-row ⋯ trigger (32 r8) + overflow menu · destructive item in error |
| Swipe action | `swipe-action.html` | Swipe-to-reveal row action (Slidable) · error panel · 88px · 40% snap (closest-token build) |
...
- `BlockCard`, `StatChip`, `DestinationCard`, `FilterChipRow`, and `PageHeader` are now canonical preview cards (see the component table). The remaining unbuilt pattern is the BottomNav filled/outline icon pairs (needs icon assets). See `MOBILE_DRIFT.md`.
- **Global header (web) carries no page title.** As of 2026-06, the desktop `Header` no longer renders a per-route title (it lives in the content-pane page header). The header's left zone is reserved for system messages / status via a `systemMessage` prop (subtitleSmall · on-surface-variant · single-line · empty by default). The right zone (search / bell / avatar) is unchanged.
